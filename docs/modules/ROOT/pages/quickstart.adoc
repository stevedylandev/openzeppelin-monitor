...
[NOTE]
====
A websocket RPC URL (wss://) is required in the network configuration to monitor transaction status. This is because transaction status information is retrieved through Substrate events, which are only available via websocket connections.
====
