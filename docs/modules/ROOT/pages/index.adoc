...
* Input: The filter script receives a JSON object containing two fields: `monitor_match` and `args`. The `monitor_match` field contains the monitor match data and the `args` (strings separated by commas) field contains the arguments of the script.
...
* All filters against a monitor match must return `false` for it to be included
...
|arguments
|String
|The arguments of the script. If you want to pass more than one argument, each argument should be separated by a comma.
